...
export const EnvironmentCountrySummary = (props: Props) => {
  const {
    data,
    indicators,
  } = props;
  return (
    <div className='margin-bottom-07'>
      <div className='flex-div flex-wrap stat-container'>
        ...
        {
          data.indicators.findIndex((d) => d.indicator === 'Emission levels associated with GHG target set by (I)NDCs') !== -1
            ? (
              <LineChart
                data={
                  data.indicators[
                ...
                source={indicators[indicators.findIndex((d) => d.DataKey === 'Emission levels associated with GHG target set by (I)NDCs')].DataSourceName}
                graphDescription={indicators[indicators.findIndex((d) => d.DataKey === 'Emission levels associated with GHG target set by (I)NDCs')].IndicatorDescription}
              />
            ) : null
        }
        {
          data.indicators.findIndex((d) => d.indicator === 'Reduced Mean Daily CO2 Emissions; percent change') !== -1
            ? (
              <LineChart
                data={
                  data.indicators[
                ...
                suffix='%'
                source={indicators[indicators.findIndex((d) => d.DataKey === 'Reduced Mean Daily CO2 Emissions; percent change')].DataSourceName}
              />
            ) : null
        }
        {
