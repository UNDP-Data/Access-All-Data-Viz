...
import {
  CtxDataType, CountryGroupDataType, HoverDataType, IndicatorMetaDataWithYear,
} from '../../Types';
...
export const SingleAxisGraph = (props: Props) => {
  const {
    data,
    indicators,
    svgWidth,
  ...
  const countryData = data[data.findIndex((d) => d['Country or Area'] === country)];
                        ...
                        d.param !== undefined ? (
                          <g>
                            <circle
                              cx={x(d.year)}
                              cy={y1(d.param)}
                              r={window.innerWidth > 960 ? 4 : window.innerWidth > 600 ? 3 : 2}
                              fill={UNDPColorModule.graphMainColor}
                            />
                            {
                              showLabel
