...
const getAnnualGrowth = (data: IndicatorDataType[], dataKey: string) => {
  ...
  if (data[indx].yearlyData.length === 0) return null;
  const { year } = data[indx].yearlyData[data[indx].yearlyData.length - 1];
  const prevYearIndx = data[indx].yearlyData.findIndex(
    d => d.year === year - 1,
  );
...
export function CountrySummary(props: Props) {
  ...
  const population = {
    ...
    annualGrowth: getAnnualGrowth(
      dataFiltered as IndicatorDataType[],
      'Population, total',
    ),
  };
  const gdp = {
    ...
    <div className='margin-bottom-13'>
      <div className='flex-div flex-space-between flex-wrap margin-bottom-07 flex-vert-align-center'>
        <h4 className='undp-typography bold margin-bottom-00'>
          Selected Indicators for {countryName}
        </h4>
        <button
      ...
      <div className='flex-div flex-wrap stat-container'>
        {population.value ? (
          <div
            className='stat-card'
            style={{ width: 'calc(33.33% - 4.67rem)', minWidth: '20rem' }}
          >
            <div>
              <h3>{format('.3s')(population.value).replace('G', 'B')}</h3>
              {population.annualGrowth ? (
                <p
                  className='bold large-font'
                  style={{
                    color:
                      population.annualGrowth < 0
                        ? 'var(--dark-red)'
                        : 'var(--dark-green)',
                  }}
                >
                  Annual Growth: {population.annualGrowth.toFixed(2)}%
                </p>
              ) : null}
            </div>
            <p className='small-font' style={{ color: 'var(--gray-500)' }}>
              {population.year}
            </p>
            <p>
              {
                indicators[
                  indicators.findIndex(el => el.DataKey === 'Population, total')
                ].IndicatorLabelTable
              }
            </p>
          </div>
          ...
          >
            <div>
              ...
              {gdp.annualGrowth ? (
                ...
                >
                  Annual Growth: {gdp.annualGrowth.toFixed(2)}%
                </p>
              ) : null}
            </div>
            <p className='small-font' style={{ color: 'var(--gray-500)' }}>
            ...
            <p>
              {
                indicators[
                  indicators.findIndex(
                    el => el.DataKey === 'Human development index (HDI)',
                  )
                ].IndicatorLabelTable
              }
            </p>
            ...
            <p>
              {
                indicators[
                  indicators.findIndex(
                    el => el.DataKey === 'GINI index (World Bank estimate)',
                  )
                ].IndicatorLabelTable
              }
            </p>
          ...
          >
            <div>
              ...
              {ghg.annualGrowth ? (
                ...
                >
                  Annual Growth: {ghg.annualGrowth.toFixed(2)}%
                </p>
              ) : null}
            </div>
            <p className='small-font' style={{ color: 'var(--gray-500)' }}>
            ...
            <p>
              {
                indicators[
                  indicators.findIndex(el => el.DataKey === 'GHG emission')
                ].IndicatorLabelTable
              }
            </p>
          </div>
